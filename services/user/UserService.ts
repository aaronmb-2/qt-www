...
import { ReturnHandleResponse, TypePromiseApiResponse } from '../response/TypesApiResponseHandler';
import { IUserService } from './IUserService';
import { ParamsUserLoginApi, ParamsUserRegisterApi, ParamsUserVerifyEmailApi } from './TypesUserService';
import { clearPersistedStore, hydrateStore, isHydrated, makePersistable } from 'mobx-persist-store';
...
class UserService implements IUserService {
  ...
  constructor() {
    ...
    makePersistable(this, {
      name: "UserService",
      properties: ["loggedInUserAccessToken", "loggedInUserRefreshToken", "loggedInUserAccountStatus"],
  ...
  async userVerifyEmail(data: ParamsUserVerifyEmailApi): TypePromiseApiResponse {
    const fetch = useCustomFetch()
  ...
  _handleSuccessfullAccessTokenLogin(apiResponse: any, message: ReturnHandleResponse) {
    toastMessageService.addToast(
      new ToastMessage({ id: Math.random(), title: message.title, message: message.message, status: message.status})
    );
  ...
  _redirectSuccessfullLogin(account_status: string) {
    const localePath = useLocalePath();
    if (account_status === EAccountStatus.verified) {
      return navigateTo({
  ...
  _handleUnsuccessfullLogin(responseMessage: ReturnHandleResponse) {
    this.logout()
    const [buttons, message] = this._errorResponseMessages(responseMessage.message)
